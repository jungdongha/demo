...
class QuantScoringServiceTest {
    ...
    class ScoreCalculation {
        ...
        void 거래량_급증시_점수가_높다() {
            ...
            assertThat(score.totalScore()).isGreaterThan(40.0); // volume 35% 기여
        ...
        void BULL_국면에서_보너스_점수가_적용된다() {
            ...
            QuantFeatureInput inputBull     = new QuantFeatureInput("AAPL", "Apple", MarketType.USA, prices, List.of(), 1.0);  // BULL
            QuantFeatureInput inputSideways = new QuantFeatureInput("AAPL", "Apple", MarketType.USA, prices, List.of(), -1.0); // SIDEWAYS

            // when
            QuantScore scoreBull     = quantScoringService.score(inputBull);
            QuantScore scoreSideways = quantScoringService.score(inputSideways);

            // then: BULL이 SIDEWAYS보다 +5점 높아야 함
        ...
        void 점수는_0에서_100_사이다() {
            ...
            QuantFeatureInput input = new QuantFeatureInput("NVDA", "NVIDIA", MarketType.USA,
                    prices, lotsOfNews, 5.0); // STRONG_BULL
    ...
    class RiskFilterTests {
        ...
        void CRISIS_국면_판단() {
            // given — 지수 5일 변동률 -6%
            double indexReturn = -6.0;

            // when
            MarketRegime regime = marketRegimeService.analyze(indexReturn);
    ...
    class Top3Selection {
        ...
        void 필터통과_2개면_2개만_반환한다() {
            ...
            QuantFeatureInput input1 = new QuantFeatureInput("005930", "삼성전자", MarketType.KOR, goodPrices, List.of("뉴스"), 1.0);
            QuantFeatureInput input2 = new QuantFeatureInput("000660", "SK하이닉스", MarketType.KOR, goodPrices, List.of("뉴스"), 1.0);
        ...
        void 점수_높은_종목이_상위에_오른다() {
            // given — 뉴스 많은 종목(newsFreshness 높음) vs 뉴스 없는 종목
            List<DailyQuote> prices = buildFlatPrices(1_000_000L, 5);
            QuantFeatureInput highNewsInput = new QuantFeatureInput("AAPL", "Apple", MarketType.USA,
                    prices, List.of("n1","n2","n3","n4","n5","n6","n7","n8","n9","n10"), 1.0);
            QuantFeatureInput noNewsInput   = new QuantFeatureInput("MSFT", "Microsoft", MarketType.USA,
                    prices, List.of(), 1.0);

            QuantScore highScore = quantScoringService.score(highNewsInput);
            QuantScore lowScore  = quantScoringService.score(noNewsInput);

            // when
            List<QuantScore> top3 = quantScoringService.selectTop3(List.of(lowScore, highScore));

            // then
            assertThat(top3.get(0).ticker()).isEqualTo("AAPL"); // 뉴스 많은 종목이 1위
        }
    }
